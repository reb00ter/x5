<template>
    <li>
        <div v-if="leaf.children" class="tree_switch"><div class="expander" :class="{opened: leaf.opened}" @click="leaf.opened = !leaf.opened"></div></div>
        <input type="checkbox" :id="leaf.type+'_'+leaf.id" @change="changed" v-model="leaf.checked">
        <label :for="leaf.type+'_'+leaf.id">{{ leaf.label }}</label>
        <ul v-if="leaf.children" v-show="leaf.opened">
            <tree_item v-for="child in leaf.children" :key="child.type+child.id" v-bind:leaf="child" @change="changed" v-if="child.visible" :title="child.type+child.id"/>
        </ul>
    </li>
</template>

<script>
    export default {
        name: "tree_item",
        props: [
            'leaf'
        ],
        methods: {
            changed:function () {
                this.$emit('change')
            }
        }
    }
</script>

<style scoped>
    .tree_switch{
        display: inline-block;
    }
    .expander{
        width: 14px;
        height: 14px;
        background-image: url("data:image/svg+xml;base64,");
    }
    .expander.opened{
        background-image: url("data:image/svg+xml;base64,");
    }
    li{
        list-style-type: none;
        margin-left: 15px;
    }
</style>