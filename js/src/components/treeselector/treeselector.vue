<template>
    <div class="tree-selector">
        <ul class="selection" @click="set_focus">
            <li class="selected-item" v-for="item in selected" :key="item.id" @click="uncheck(item)">{{ item.label }}</li>
            <li class="input"><input type="text" v-model="query" title="фильтр"></li>
        </ul>
        <ul class="suggestions" :style="suggestion_size">
            <tree_item v-for="elem in filtered_options" :key="elem.type+elem.id" v-bind:leaf="elem" @change="update_selected" v-if="elem.visible"/>
        </ul>
    </div>
</template>

<script>
    import tree_item from './tree_item.vue'
    import Vue from "vue";
    ...
    function filterTree(root, expression) {
        if(root.label.match(new RegExp(expression, "i"))){
    ...
    export default {
        ...
        computed:{
            filtered_options: function () {
                if(this.options)
                {
                    let query_parts = this.query.split(/ |,|\./g)
                    let expression = query_parts.join('.*')
                    let result = []
